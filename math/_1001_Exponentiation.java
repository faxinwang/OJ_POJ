...
magnitude and precision are common. For example, the computation 
of the national debt is a taxing experience for many computer systems. 

This problem requires that you write a program to compute the 
exact value of Rn where R is a real number ( 0.0 < R < 99.999 ) 
and n is an integer such that 0 < n <= 25.
Input

The input will consist of a set of pairs of values for R and n. 
The R value will occupy columns 1 through 6, and the n value 
...
output. Insignificant trailing zeros must not be printed. Don't 
...
public class _1001_Exponentiation {
	...
	public static void main(String[] args) {
		...
		while(sc.hasNextLine())
		{
			...
			for(int i=0; i<n; ++i) ans = ans.multiply(r); //计算r^n
			String s = ans.stripTrailingZeros() //除去末尾的0
					.toPlainString(); //不用科学计数法表示,用普通的小数形式表示
			if(s.charAt(0) == '0') s = s.substring(1); //如果整数部分是0, 则忽略前导0
